...
pub fn dout(ir: &mut IRGenUtil) {
    ...
    let fmt_str = ir.builder.build_global_string_ptr("%d", "").expect("Failed to build global string").as_pointer_value();
    ...
    ir.builder.build_call(ir.extract_microcode_function("printf").unwrap(), &[fmt_str.into(), loaded_val.into()], "").unwrap();
...
pub fn nl(ir: &mut IRGenUtil) {
    ...
    let fmt_str = ir.builder.build_global_string_ptr("\n", "").expect("Failed to build global string").as_pointer_value();
    ir.builder.build_call(ir.extract_microcode_function("printf").unwrap(), &[fmt_str.into()], "").unwrap();
