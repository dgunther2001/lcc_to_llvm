...
pub fn dout(ir: &mut IRGenUtil) {
    ...
    let sr = dout_fn.get_nth_param(0).unwrap().into_pointer_value();

    sr.set_name("sr");

    let fmt_str = ir.builder.build_global_string_ptr("%d", "dout_fmt").expect("Failed to build global string").as_pointer_value();
    let sr_val = ir.builder.build_load(ir.context.i16_type(), sr, "sr_val").expect("Failed to load from register");
    //let zext_val = ir.builder.build_int_z_extend(sr_val.into_int_value(), ir.context.i32_type(), "zext_val").expect("Failed to build zero extended i32 for DOUT");

    ir.builder.build_call(ir.extract_microcode_function("printf").unwrap(), &[fmt_str.into(), sr_val.into()], "printf_call").unwrap();
...
pub fn nl(ir: &mut IRGenUtil) {
    let nl_fn_type = ir.context.void_type().fn_type(&[], false);
